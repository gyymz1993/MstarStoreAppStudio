...
public class PhotoActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < PostCommentActivity.imgList.size(); i++) {
			bmp.add(PostCommentActivity.imgList.get(i));
		}

		for (int i = 0; i < PostCommentActivity.filePaths.size(); i++) {
			drr.add(PostCommentActivity.filePaths.get(i));
		}
		...
		photo_bt_del.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (listViews.size() == 1) {
					PostCommentActivity.imgList.clear();
					BimpUtils.getInstace().drr.clear();
					finish();
				} else {
					bmp.remove(count);
					drr.remove(count);
		...
		photo_bt_enter.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				PostCommentActivity.imgList = bmp;
				PostCommentActivity.filePaths = drr;
				finish();
			}
		});
